...
################################################################################
### LURK-Vecchia: Spatial Land-use Regression Kriging with Vecchia Approximation
################################################################################

LURK_Spatial_Vecchia <- function(Y,X,locs,covparams = NULL,beta.hat = NULL, tol = NULL, m = NULL){
  ...
  if(is.null(covparams)){
    d.sample <- sample(1:N,N/50,replace = FALSE)
    D.sample = rdist(locs[d.sample,1:2])
    covparams <- c(.9*var(Y),mean(D.sample)/4,0.1*var(Y)) 
  }  
  if(is.null(beta.hat)){
    beta.hat <- rep(0,ncol(X))
  }
  ...
  while(!converged){
    ...
    # estimate theta
    vecchia.result<- optim(par=log(covparams),fn=negloglik_vecchia,
                           locs= locs, res=res,vecchia.approx=vecchia.approx,method = "Nelder-Mead",
                           control=list(trace=0))
    ...
    transformed.data=transform_iid(cbind(Y,as.matrix(X)),
                                   vecchia.approx = vecchia.approx,c(covparams[1],covparams[2],0.5),covparams[3])
